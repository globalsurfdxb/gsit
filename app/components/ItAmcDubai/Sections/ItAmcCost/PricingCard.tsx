    ...
    >
      ...
      <div className="flex items-center gap-2 flex-wrap mb-5  mb:mb-6">
        {tags.map((tag, i) => (
          <span
            key={i}
            className={`text-[11.5px] font-semibold leading-[1] px-2 py-[6.5px] rounded-[6px] transition-colors duration-500 ${
              dark ? "bg-white/15 text-white" : "bg-white text-paragraph"
            }`}
          >
            {tag}
          </span>
      ...
      <hr className={`mb-5 transition-colors duration-500 ${dark ? "border-white/20" : "border-[#D3D3D3]"}`} />
