    ...
    >
      ...
      <h3
        className={`text-32px transition-colors duration-300 ${
          active ? "text-primary" : "text-paragraph group-hover:text-primary"
        }`}
      >
        {title}
      </h3>
      ...
      {active && description && (
        <p className="text-paragraph text-[15px] md:text-[16px] font-light mt-4 max-w-[50ch]">
          {description}
        </p>
      )}

      </div>
      <hr className={`border-t ${active ? "border-primary" : "border-transparent"}`} />
    </button>
  );
}