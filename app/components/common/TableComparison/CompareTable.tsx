...
import TechMediumTable, { type ColumnWidth, type TableColumn, type TableRow } from "./Table";
  ...
  return (
    <section className="bg-white rounded-2xl py-82">
      <div className="container">
        ...
        <TechMediumTable columns={data.columns} rows={data.rows} columnwidth={columnwidth} />
        ...
        {data.roomreading && (
          <div className="pt-6 lg:pt-10.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 lg:gap-x-7.5 gap-y-6 lg:gap-y-8 mt-4 lg:mt-6 2xl:mt-10.5">
              {data.roomreading.items?.map((item, index) => (
                <div key={index} className="flex items-start gap-4">
                  <span className="mt-[6px] shrink-0 w-[12px] h-[12px] bg-primary" />
                  <p className="text-paragraph text-18 tracking-[-3%] m-0">{item}</p>
                </div>
              ))}
            </div>
          </div>
